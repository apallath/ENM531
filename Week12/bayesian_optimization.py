...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 20 14:29:56 2018
...
if __name__ == "__main__":    
    ...
    for i in range(0,nsteps):
        ...
        dlf = np.linalg.norm(model.X[-1,:] - new_X, 2)/np.linalg.norm(model.X[-1,:],2)
        if dlf < tol:
            print("Converged!")
            break
        ...
        plt.plot(new_X*np.ones((2,1)), np.linspace(ymin,ymax,2),'k--')
        ax.set_xlim([lb[0], ub[0]])
        plt.xlabel('$x$')
        plt.ylabel('$EI(x)$')
        plt.pause(0.5)
